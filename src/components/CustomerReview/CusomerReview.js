...
const CusomerReview = () => {
  return (
    <Container>
        ...
        <div className='mt-[5%]  flex justify-center gap-5  h-400px items-center align-middle'>
            {CustmerData.map((data,key)=>(
                <div className='flex justify-center box font-poppins capitalize flex-col rounded-md items-center gap-5
                 ...
                 hover:bg-indigo-500 hover:text-white'>
                   <div className='bg-transparent flex gap-2 absolute left-0 top-0 pt-2 pl-2 justify-center align-middle items-center  rounded-full  '>
                    ...
                    <h1 className='text-black text-[16px] font-semibold'>{data.name}</h1>
                 
                  </div>
                  <div className='flex flex-col gap-1 justify-center mt-5 items-center'>
                     <FaQuoteLeft color='slate'/>
                    <p className=' w-[300px]'>{data.review}</p>
                  </div>
            </div>  
            ))}
